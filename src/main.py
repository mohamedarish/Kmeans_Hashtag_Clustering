...
CONSUMER_KEY = os.getenv("CLIENT_KEY")
...
# Specify the username and the number of tweets to retrieve
twitter_username = "marishtp"
num_tweets = 100

# Retrieve tweets
tweets = tweepy.Cursor(
    api.user_timeline, screen_name=twitter_username, tweet_mode="extended"
).items(num_tweets)

# Create a CSV file to store the tweets
csv_file = open("tweets.csv", "w", encoding="utf-8", newline="")
csv_writer = csv.writer(csv_file)
csv_writer.writerow(["Tweet ID", "Created At", "Text"])
...
print(f"Tweets from {twitter_username} saved to tweets.csv")

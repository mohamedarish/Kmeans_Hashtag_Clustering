...
def process_rdd(time, rdd):
    ...
    try:
        ...
        print(
            "Get spark sql singleton context from the current context ----------- %s -----------"
            % str(time))
        ...
        hashtag_counts_df.coalesce(1).write.format(
            "com.databricks.spark.csv").mode("overwrite").option(
                "header", "true").csv("/Users/girishdurgaiah/hashtag_file.csv")
        ...
        country_counts_df.coalesce(1).write.format(
            "com.databricks.spark.csv").mode("overwrite").option(
                "header", "true").csv("/Users/girishdurgaiah/country_file.csv")
        ...
        device_df.show()
        device_df.coalesce(1).write.format(
            "com.databricks.spark.csv").mode("overwrite").option(
                "header", "true").csv("/Users/girishdurgaiah/device_file.csv")
